...
while aws cloudformation list-stacks | jq -r '.StackSummaries[] | if .StackStatus != "UPDATE_COMPLETE" and .StackStatus != "UPDATE_ROLLBACK_COMPLETE" then .StackName + ": " + .StackStatus else "" end' | grep -v 'DELETE_COMPLETE' | grep -s ignorant ; do
  sleep 1
done
